...
async def test_client():
    ...
    return EWTestClient(app), container
...
@pytest.fixture
async def create_user(test_client):
    container = test_client[1]
    user_data = {
        "username": "testuser",
        "password": "password123"
    }
    orm_service: ORMService = await container.get('ORMService')
...
async def test_framework_GET_root_http_request_needs_CSRF(test_client, test_db_cleanup):
    response = await test_client[0].get("/")
...
async def test_login_valid_credentials(test_client, create_user, test_db_cleanup):
    response = await test_client[0].get("/register")
    assert response.status_code == 200, "Expected status code 200."

    csrf_token = extract_csrf_token(response.body)

    response = await test_client[0].post(
        "/register",
        form_data={
            "username": "newuser",
    ...
    csrf_response = await test_client[0].get("/login")
...
async def test_session_expiry(test_client, create_user):
    ...
    response = await test_client[0].get("/hello", cookies={"session_id": session_id})
